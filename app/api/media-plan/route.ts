...
export async function POST(request:Request){
  ...
  if(!apiKey)return NextResponse.json({...base,error:'missing_openai_api_key'})
  ...
  try{
    ...
    for(const image of images){
      ...
      content.push({type:'input_image',image_url:image.url,detail:'high'})
    }
    const response=await fetch('https://api.openai.com/v1/responses',{
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':`Bearer ${apiKey}`},
      body:JSON.stringify({model:'gpt-5',input:[{role:'user',content}],text:{verbosity:'medium'}})
    })
    if(!response.ok){
      const detail=await response.text().catch(()=> '')
      console.error('media-plan OpenAI error',response.status,detail.slice(0,500))
      return NextResponse.json({...base,error:`openai_http_${response.status}`})
    }
    ...
    console.error('media-plan exception',error)
    return NextResponse.json({...base,error:'openai_exception'})
  }
}
