...
export async function POST(request: Request){
  ...
  if(!apiKey) return NextResponse.json({...fallback,error:'missing_openai_api_key'})
  ...
  try{
    ...
    const response = await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({model:'gpt-5',input:prompt,text:{verbosity:'low'}})
    })
    if(!response.ok){
      const detail=await response.text().catch(()=> '')
      console.error('ai-draft OpenAI error',response.status,detail.slice(0,500))
      return NextResponse.json({...fallback,error:`openai_http_${response.status}`})
    }
    ...
    console.error('ai-draft exception',error)
    return NextResponse.json({...fallback,error:'openai_exception'})
  }
}
